...
(async () => {
  const url = "https://everymac.com/systems/apple/macbook_pro/index-macbookpro.html";
  ...
  const macbookData = await page.evaluate(() => {
    const models = [];
    const containers = document.querySelectorAll("div[id^='contentMacBookPro']");
  ...
  fs.writeFileSync("macbookPro_models.json", JSON.stringify(macbookData, null, 2));
  console.log("Datos guardados en macbook_modelsPro.json");
